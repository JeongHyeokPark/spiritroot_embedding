...
void STSingleTrackGenerator::ReadConfig(const std::string& t_config)
{
  ...
  if(parser.KeyExist("RandomPt"))
  {
    auto bound = parser.GetBound("RandomPt");
    SetRandomPt(true, bound.first, bound.second);
  }

  if(parser.KeyExist("RandomRapidity"))
  {
    auto bound = parser.GetBound("RandomRapidity");
    SetRandomRapidity(true, bound.first, bound.second);
  }
  ...
  if(parser.KeyExist("RandomLCP"))
  {
    auto list = parser.Get<bool>("RandomLCP");
    if( list )
    {
      SetParticleList( {2212, 1000010020, 1000010030, 1000020030, 1000020040} );
      SetRandomPt(true);
    }
  }
  ...
  else
  {
    do 
    {
      if(fRandomMomentum){
        if( fRandomPt )
        {
          if( pdg == 2212 ) fMomentumRange[1] = 1.5;
          if( pdg == 1000010020 ) fMomentumRange[1] = 2.4;
          if( pdg == 1000010030 ) fMomentumRange[1] = 3.5;
          if( pdg == 1000020030 ) fMomentumRange[1] = 3.5;
          if( pdg == 1000020040 ) fMomentumRange[1] = 4.0;
        }
      ...
      if(fRandomPt && fRandomRapidity){
          Double_t mass = 0.;
          if( pdg == 2212 ) mass = 0.938;
          if( pdg == 1000010020 ) mass = 1.8756;
          if( pdg == 1000010030 ) mass = 2.80892;
          if( pdg == 1000020030 ) mass = 2.80839;
          if( pdg == 1000020040 ) mass = 3.72738;

        Double_t pt = gRandom->Uniform(fPtRange[0], fPtRange[1]);
        Double_t rap = gRandom->Uniform(fRapRange[0], fRapRange[1]); // LAB

        Double_t mt = TMath::Sqrt( mass*mass + pt*pt );
        Double_t pz = mt * TMath::SinH( rap );
        Double_t randomPhi = gRandom->Uniform( -TMath::Pi(), TMath::Pi() );
        Double_t px = pt * TMath::Cos( randomPhi );
        Double_t py = pt * TMath::Sin( randomPhi );
