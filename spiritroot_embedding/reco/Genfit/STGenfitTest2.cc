...
STGenfitTest2::STGenfitTest2(bool loadSamurai, Double_t xOffset, Double_t yOffset, Double_t zOffset)
{
  fTPCDetID = 0;
  ...
  auto fieldPointer = STGFBField::GetInstance("samurai_field_map", "A", xOffset, yOffset, zOffset);
...
bool STGenfitTest2::ExtrapolateTo(genfit::Track *gfTrack, TVector3 to, TVector3 &position)
{
  ...
  try {
    trackRep -> extrapolateToPoint(fitState, .1*to, kTRUE);
  } catch (genfit::Exception &e) {
  }
