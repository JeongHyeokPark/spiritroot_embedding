...
//      double ProjectedAtZ=-592.644;//////mid target = -592.644, start pad plane =-580.4, end of pad plane = 763.6
  ...
  LOG(INFO) << Space() << "STGenfitVATask pripos " << fMCEventHeader -> GetX()*10.  << " " << fMCEventHeader -> GetY()*10. << " " << fMCEventHeader -> GetZ()*10. << FairLogger::endl;
...
//    cout<<" Phi: "<<Phi<<" Theta: "<<Theta<<" ShiftX: "<<ShiftX<<" ShiftY: "<<ShiftY<<endl;
    ...
    for (Int_t iPID = 0; iPID < NUMSTPID; ++iPID)
    {
      ...
      if( pgfTrack!=nullptr ) fGenfitTest -> GetTrackParameters(pgfTrack, pmom, pmomTargetPlane, pposTargetPlane);
