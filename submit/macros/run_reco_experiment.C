...
)
{
  ...
  if (start + fNumEventsInSplit > fNumEventsInRun)
    ...
    std::cout << fSplitNo << " " << fNumEventsInSplit << " " << start << std::endl;
  ...
  genfitVA -> SetBeamFile(Form("/data/RB230064/common/BeamData/beam_run%d.ridf.root", fRunNo));
