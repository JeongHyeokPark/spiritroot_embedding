...
)
{
  ...
  TString genDir   = workDir + "/macros/genRap_Sn108/";
  //  TString genDir   = workDir + "/macros/genRap_Sn108_left35/";
  ...
  TString genFile;
  if(2212 == pdg)
    genFile = genDir + Form("%02d/mc%06d_1A.root",bulknum/10000,bulknum);
  else if(1000010020 == pdg)
    genFile = genDir + Form("%02d/mc%06d_2A.root",bulknum/10000,bulknum);
  else if(1000010030 == pdg)
    genFile = genDir + Form("%02d/mc%06d_3A.root",bulknum/10000,bulknum);
  else if(1000020030 == pdg)
    genFile = genDir + Form("%02d/mc%06d_3A.root",bulknum/10000,bulknum);
  else if(1000020040 == pdg)
    genFile = genDir + Form("%02d/mc%06d_4A.root",bulknum/10000,bulknum);
...
//  AddIons(fRun, event);
  ...
  //  fEvent -> SetPrimaryVertex(0, -22.99, -1.34); // Sep. 7th, 2018 for Sn108
  fEvent -> SetPrimaryVertex(-0.0706, -20.55-2.328, -1.2244); // Jan. 14th, 2020 for fva Sn108
