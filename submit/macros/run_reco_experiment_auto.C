...
)
{
  ...
  fParameterFile = fParamSetter -> GetParameterFile();
  ...
  TString fname = "";
  for( int i=22; i<fMCFile.Length(); i++ ) fname += fMCFile[i];
  if( !fMCFile.IsNull() ) out = fPathToData+"run"+sRunNo+"_s"+sSplitNo+"."+fname+".reco."+version+".root";
  if( !fMCFile.IsNull() ) log = fPathToData+"run"+sRunNo+"_s"+sSplitNo+"."+fname+"."+version+".log";
